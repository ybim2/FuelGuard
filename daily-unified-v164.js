(() => {
  ...
  function enhancePatterns() {
    ...
    if (selectedAll) {
      ...
      root.querySelectorAll(".beta-fuelling-pattern-chart-card,.athlete-training-pattern-lanes").forEach(node=>node.remove());
      const items=allTimelineData(); const signature=timelineSignature(items); const existing=root.querySelector(".fg-all-timeline");
      if (!existing) nav.insertAdjacentHTML("afterend",allTimelineMarkup(items,signature));
      else if (existing.dataset.fgSignature!==signature) existing.outerHTML=allTimelineMarkup(items,signature);
      return;
    }

    root.querySelector(".fg-all-timeline")?.remove();
    const active = nav.querySelector("button.active")?.dataset.logPatternType;
    if (active === "supplements") {
      const card=root.querySelector(".beta-fuelling-pattern-chart-card"); if (!card) return;
      const items=supplementPlanData(); const signature=supplementSignature(items); const existing=card.querySelector("[data-fg-planned-supplements]");
      if (!existing) card.insertAdjacentHTML("beforeend",plannedSupplementMarkup(items,signature));
      else if (existing.dataset.fgSignature!==signature) existing.outerHTML=plannedSupplementMarkup(items,signature);
    }
  }

  function refreshNow() { scheduled=false; moveRoutines(); enhancePatterns(); }
  function refresh() { if (scheduled) return; scheduled=true; requestAnimationFrame(refreshNow); }

  document.addEventListener("click", event => {
    const tab=event.target.closest?.("[data-log-pattern-type]"); if (!tab) return;
    if (tab.dataset.logPatternType==="all") { event.preventDefault(); event.stopImmediatePropagation(); selectedAll=true; refresh(); }
