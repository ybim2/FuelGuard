...
async function findWorkModeSession(event, userId, env) {
  const params = new URLSearchParams({
  ...
  const result = await supabaseRequest(`/rest/v1/fuel_work_mode_sessions?${params.toString()}`, {
    method: "GET",
    headers: { Accept: "application/json" }
  }, env);
  if (!result.response.ok) {
    // Work Mode is additive. Garmin logging remains available while a release
    // is being deployed in migration-first order.
    if (result.response.status === 404 || result.data?.code === "PGRST205") return null;
    throw new Error("Work Mode lookup failed.");
  }
  const loggedAt = new Date(event.logged_at).getTime();
  return (Array.isArray(result.data) ? result.data : []).find(session => {
    const start = new Date(session.started_at).getTime();
...
async function insertGarminLog(event, userId, env) {
  const [trainingSession, workSession] = await Promise.all([
    findTrainingModeSession(event, userId, env),
    findWorkModeSession(event, userId, env)
  ]);
  const row = {
    user_id: userId,
    ...
    work_mode_session_id: workSession?.id || undefined,
...
async function garminTrainingHandler(request, response) {
  ...
  if (request.method === "GET") {
    try {
      const session = await activeTrainingModeSession(device.user_id, env);
      return jsonResponse(response, 200, {
        result: "ok",
