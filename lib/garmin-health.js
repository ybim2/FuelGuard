...
function requestedLocalDate(request, features, env) {
  let value = "";
  try {
    const url = new URL(request.url || "/", "https://fuel.guard");
    value = url.searchParams.get("date") || "";
  } catch {
    value = "";
  }
  if (/^\d{4}-\d{2}-\d{2}$/.test(value)) return value;
  const latestFeature = Array.isArray(features) && features.length ? features[features.length - 1] : null;
  if (latestFeature?.local_date) return latestFeature.local_date;
  return localDateKey(testNow(env), "UTC");
}
...
async function selectedDayDetails(userId, localDate, features, env) {
  const feature = (Array.isArray(features) ? features : []).find(row => row.local_date === localDate) || null;
  const timezone = feature?.timezone || "UTC";
  const range = wideDateRange(localDate);
  const filters = { user_id: userId };
  const [fuelRows, heartRows, stressRows, bodyBatteryRows, activityRows] = await Promise.all([
    fetchRangeRows("fuel_logs", "logged_at,type,source", filters, "logged_at", range, env),
    fetchRangeRows("garmin_heart_rate_samples", "observed_at,value_bpm", filters, "observed_at", range, env),
    fetchRangeRows("garmin_stress_samples", "observed_at,value,sample_status", filters, "observed_at", range, env),
    fetchRangeRows("garmin_body_battery_samples", "observed_at,value", filters, "observed_at", range, env),
    fetchRangeRows("garmin_activity_summaries", "started_at,activity_type,duration_seconds,distance_metres", filters, "started_at", range, env)
  ]);
  ...
  return {
    ...
    timezone,
    feature,
    fuel_logs: serializeRows(localDayRows(fuelRows, "logged_at", localDate, timezone)
      .filter(row => row.type === "fuel" || row.type === "fuel_hydration"), row => ({
        logged_at: row.logged_at,
        type: row.type,
        source: row.source
      })),
    heart_rate_samples: serializeRows(localDayRows(heartRows, "observed_at", localDate, timezone), row => ({
      observed_at: row.observed_at,
      value_bpm: row.value_bpm
    })),
    stress_samples: serializeRows(localDayRows(stressRows, "observed_at", localDate, timezone), row => ({
      observed_at: row.observed_at,
      value: row.value,
      sample_status: row.sample_status
    })),
    body_battery_samples: serializeRows(localDayRows(bodyBatteryRows, "observed_at", localDate, timezone), row => ({
      observed_at: row.observed_at,
      value: row.value
    })),
    activity_summaries: serializeRows(localDayRows(activityRows, "started_at", localDate, timezone), row => ({
...
async function garminPatternsHandler(request, response) {
  ...
  try {
    ...
    const selectedDate = requestedLocalDate(request, features, env);
    const selectedDay = await selectedDayDetails(user.id, selectedDate, features, env);
    return jsonResponse(response, 200, {
      source: SOURCE,
