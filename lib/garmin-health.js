...
const TRAINING_PRE_FUEL_WINDOW_MINUTES = 180;
const TRAINING_POST_FUEL_WINDOW_MINUTES = 90;
const TRAINING_LONG_GAP_OVERLAP_MINUTES = 300;
const TRAINING_COVERAGE_WORKOUT_LIMIT = 8;
...
function trainingFuelSummary(fuelLogs, activities) {
  ...
  const preWindowMs = TRAINING_PRE_FUEL_WINDOW_MINUTES * 60 * 1000;
  const postWindowMs = TRAINING_POST_FUEL_WINDOW_MINUTES * 60 * 1000;
  let before = 0;
  let after = 0;
  let missingBefore = 0;
  let missingAfter = 0;
...
function nearestFuelBefore(fuelDates, start) {
  return fuelDates.filter(date => date <= start).sort((a, b) => b - a)[0] || null;
}

function firstFuelAfter(fuelDates, end) {
  return fuelDates.filter(date => date >= end).sort((a, b) => a - b)[0] || null;
}

function buildTrainingFuelCoverage(fuelLogs, activities, timezone = "UTC") {
  const sortedActivities = (Array.isArray(activities) ? activities : [])
    .map(activity => {
      const start = new Date(activity.started_at);
    ...
    .slice(0, TRAINING_COVERAGE_WORKOUT_LIMIT);
  const fuelDates = (Array.isArray(fuelLogs) ? fuelLogs : [])
    .map(row => new Date(row.logged_at))
    .filter(date => !Number.isNaN(date.getTime()))
    .sort((a, b) => a - b);
  const gaps = gapWindows(fuelLogs).filter(gap => gap.minutes >= TRAINING_LONG_GAP_OVERLAP_MINUTES);
  const workouts = sortedActivities.map(activity => {
    const before = nearestFuelBefore(fuelDates, activity.start);
    const after = firstFuelAfter(fuelDates, activity.end);
    const preGapMinutes = before ? Math.round((activity.start - before) / 60000) : null;
    const postGapMinutes = after ? Math.round((after - activity.end) / 60000) : null;
    const overlap = gaps.find(gap => overlaps(gap.start, gap.end, activity.start, activity.end)) || null;
    return {
      activity_type: boundedString(activity.activity_type || "activity", 48) || "activity",
      started_at: activity.start.toISOString(),
      ended_at: activity.end.toISOString(),
      local_date: localDateKey(activity.start, timezone),
      duration_minutes: Math.round(Number(activity.duration_seconds || 0) / 60),
      pre_workout_fuel_logged_at: before ? before.toISOString() : null,
      pre_workout_gap_minutes: preGapMinutes,
      pre_workout_covered: Number.isFinite(preGapMinutes) && preGapMinutes <= TRAINING_PRE_FUEL_WINDOW_MINUTES,
      post_workout_fuel_logged_at: after ? after.toISOString() : null,
      post_workout_gap_minutes: postGapMinutes,
      post_workout_covered: Number.isFinite(postGapMinutes) && postGapMinutes <= TRAINING_POST_FUEL_WINDOW_MINUTES,
      long_gap_overlapped: Boolean(overlap),
      overlap_gap_minutes: overlap ? Math.round(overlap.minutes) : null,
      status: !Number.isFinite(preGapMinutes) || preGapMinutes > TRAINING_PRE_FUEL_WINDOW_MINUTES
        ? "pre_workout_gap"
        : !Number.isFinite(postGapMinutes) || postGapMinutes > TRAINING_POST_FUEL_WINDOW_MINUTES
          ? "post_workout_gap"
          : overlap
            ? "long_gap_overlap"
            : "covered"
  ...
  const missingPre = workouts.filter(item => !item.pre_workout_covered).length;
  const postCovered = workouts.filter(item => item.post_workout_covered).length;
  const longGapOverlaps = workouts.filter(item => item.long_gap_overlapped).length;
  const summary = [];
  if (workoutCount >= 3) {
    if (missingPre > 0) {
      summary.push(`${missingPre} of your last ${workoutCount} Garmin workouts started more than ${TRAINING_PRE_FUEL_WINDOW_MINUTES / 60} hours after the previous fuel log.`);
    }
    summary.push(`${postCovered} of your last ${workoutCount} workouts had fuel logged within ${TRAINING_POST_FUEL_WINDOW_MINUTES} minutes afterward.`);
    if (longGapOverlaps > 0) {
      summary.push(`${longGapOverlaps} recent workout${longGapOverlaps === 1 ? "" : "s"} overlapped a fuelling gap longer than ${TRAINING_LONG_GAP_OVERLAP_MINUTES / 60} hours.`);
    }
  }
  return {
    pre_window_minutes: TRAINING_PRE_FUEL_WINDOW_MINUTES,
    post_window_minutes: TRAINING_POST_FUEL_WINDOW_MINUTES,
    long_gap_overlap_minutes: TRAINING_LONG_GAP_OVERLAP_MINUTES,
    workout_count: workoutCount,
    missing_pre_workout_fuel_count: missingPre,
    post_workout_covered_count: postCovered,
    long_gap_overlap_count: longGapOverlaps,
    enough_for_summary: workoutCount >= 3,
    summary,
    insufficient_data_message: workoutCount < 3
      ? `Log fuel around ${Math.max(0, 3 - workoutCount)} more Garmin workout${Math.max(0, 3 - workoutCount) === 1 ? "" : "s"} to build your first training-fuel summary.`
      : "",
    limitation: "Fuel Guard analyses logged events; unlogged food cannot be detected.",
    workouts
  };
}
...
async function latestTableRow(table, select, userId, orderColumn, env) {
  const params = new URLSearchParams({
    select,
    user_id: `eq.${userId}`,
    source: `eq.${SOURCE}`,
  ...
  if (!result.response.ok) throw new Error(`${table}_latest_lookup_failed`);
  return Array.isArray(result.data) ? result.data[0] || null : null;
}

async function garminDataStatus(userId, capabilities, env) {
  const metricLookups = [
    ["heart_rate", "Heart rate", "garmin_heart_rate_samples", "observed_at,created_at", "observed_at"],
    ["stress", "Stress", "garmin_stress_samples", "observed_at,created_at", "observed_at"],
    ["body_battery", "Body Battery", "garmin_body_battery_samples", "observed_at,created_at", "observed_at"],
    ["resting_hr", "Resting HR", "garmin_profile_snapshots", "observed_at,created_at", "observed_at"],
    ["activities", "Activities", "garmin_activity_summaries", "started_at,created_at", "started_at"]
  ];
  const received = [];
  const metricRows = {};
  for (const [key, label, table, select, orderColumn] of metricLookups) {
    const row = await latestTableRow(table, select, userId, orderColumn, env);
    metricRows[key] = row;
    if (row) received.push(label);
  }
  const latestUpload = Object.values(metricRows)
    .map(row => row?.created_at || row?.observed_at || row?.started_at)
    .filter(Boolean)
    .map(value => new Date(value))
    .filter(date => !Number.isNaN(date.getTime()))
    .sort((a, b) => b - a)[0] || null;
  const lastCollection = capabilities?.collected_at || null;
  const sharing = capabilities ? "on" : "off";
  return {
    health_sharing: sharing,
    last_watch_collection_at: lastCollection,
    last_server_upload_at: latestUpload ? latestUpload.toISOString() : null,
    metrics_received: received,
    capabilities: capabilities?.capabilities || {},
    device_id: capabilities?.device_id || null,
    data_freshness: latestUpload ? "received" : "waiting",
    message: latestUpload
      ? `${received.join(", ") || "Garmin data"} received.`
      : capabilities
        ? "Health sharing is enabled, but no supported samples have been received yet."
        : "Health sharing is off or no Garmin data has been received yet."
  };
}

async function latestTrainingCoverage(userId, env, timezone = "UTC") {
  const activityParams = new URLSearchParams({
    select: "started_at,activity_type,duration_seconds,distance_metres",
    user_id: `eq.${userId}`,
    source: `eq.${SOURCE}`,
    order: "started_at.desc",
    limit: String(TRAINING_COVERAGE_WORKOUT_LIMIT)
  });
  const activitiesResult = await supabaseRequest(`/rest/v1/garmin_activity_summaries?${activityParams.toString()}`, {
    method: "GET",
    headers: { Accept: "application/json" }
  }, env);
  if (!activitiesResult.response.ok) throw new Error("training_activities_lookup_failed");
  const activities = Array.isArray(activitiesResult.data) ? activitiesResult.data : [];
  if (!activities.length) {
    return buildTrainingFuelCoverage([], [], "UTC");
  }
  const sorted = activities
    .map(activity => ({ ...activity, start: new Date(activity.started_at), end: activityEnd(activity) }))
    .filter(activity => !Number.isNaN(activity.start.getTime()) && activity.end)
    .sort((a, b) => a.start - b.start);
  const earliest = sorted[0].start;
  const latestEnd = sorted[sorted.length - 1].end;
  const fuelRange = {
    start: new Date(earliest.getTime() - 24 * 60 * 60 * 1000).toISOString(),
    end: new Date(latestEnd.getTime() + 6 * 60 * 60 * 1000).toISOString()
  };
  const fuelLogs = await fetchRangeRows(
    "fuel_logs",
    "logged_at,type,source",
    { user_id: userId },
    "logged_at",
    fuelRange,
    env
  );
  const fuelOnly = fuelLogs.filter(row => ["fuel", "fuel_hydration"].includes(row.type));
  return buildTrainingFuelCoverage(fuelOnly, activities, timezone);
}
...
async function garminPatternsHandler(request, response) {
  ...
  try {
    ...
    const timezone = features[features.length - 1]?.timezone || "UTC";
    const healthStatus = await garminDataStatus(user.id, capabilities, env);
    const trainingFuelCoverage = await latestTrainingCoverage(user.id, env, timezone);
    return jsonResponse(response, 200, {
      source: SOURCE,
      features_count: features.length,
      capabilities,
      health_status: healthStatus,
      training_fuel_coverage: trainingFuelCoverage,
      insights,
