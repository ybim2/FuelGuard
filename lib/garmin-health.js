...
const fuelGuardDomain = require("../fuel-guard-domain.js");
...
function trainingFuelSummary(fuelLogs, activities) {
  ...
  const contexts = fuelGuardDomain.getWorkoutFuelContexts(activities.map(activity => ({
    id: activity.id || activity.source_activity_id || "",
  ...
  const longGapOverlapCount = gapWindows(fuelLogs)
    ...
    .length;
  return {
    fuel_events_before_training: contexts.filter(context => context.hasPreviousFuel).length,
    fuel_events_after_training: contexts.filter(context => context.hasPostFuel).length,
    workouts_missing_pre_fuel: contexts.filter(context => !context.hasPreviousFuel).length,
    workouts_missing_post_fuel: contexts.filter(context => !context.hasPostFuel).length,
...
async function generateDailyFeature({ userId, localDate, timezone, env }) {
  ...
  const surroundingFuelLogs = fuelLogRows.filter(row => row.type === "fuel" || row.type === "fuel_hydration");
  const fuelLogs = localDayRows(surroundingFuelLogs, "logged_at", localDate, timezone);
  ...
  const trainingFuel = trainingFuelSummary(surroundingFuelLogs, activities);
  ...
  try {
    ...
    sections.activity_summaries = await insertRows("garmin_activity_summaries", activities.rows, row => ({
      ...
      ...(row.source_activity_id
        ...
        : { device_id: row.device_id, started_at: row.started_at, activity_type: row.activity_type })
...
function buildGarminPatternInsights(rows) {
  ...
  if (workoutCount >= 5 && missingPreFuel >= 3 && missingPreFuel / workoutCount >= 0.5) {
    candidates.push(patternCandidate({
      ...
      text: `On ${missingPreFuel} of your last ${workoutCount} Garmin activities, no prior fuel event was available before starting.`,
      detail: "This is a concrete logging pattern around training, not a claim about performance.",
      metric: "training",
      count: workoutCount,
      magnitude: missingPreFuel,
      tone: "elevated",
      action: "Use the timing view to notice whether the same pre-training pattern repeats."
    }));
  }
  ...
  if (workoutCount >= 5 && missingPostFuel >= 3 && missingPostFuel / workoutCount >= 0.5) {
    candidates.push(patternCandidate({
      ...
      text: `On ${missingPostFuel} of your last ${workoutCount} Garmin activities, no subsequent fuel event was available after finishing.`,
      detail: "This can help you notice whether post-training fuel moments are getting missed.",
      metric: "training",
      count: workoutCount,
      magnitude: missingPostFuel,
      tone: "elevated",
      action: "Use the timing view to notice whether the same post-training pattern repeats."
    }));
  }
