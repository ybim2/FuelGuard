(() => {
  ...
  async function confirmRoutine(id) {
    ...
    try {
      ...
      const next = occurrences(); next[occurrenceKey(routine, now)] = { status: "confirmed", plannedAt: planned.toISOString(), confirmedAt: new Date().toISOString(), coffee: Boolean(routine.coffee), supplementPlanIds: (routine.supplementPlanIds || []).slice() }; saveOccurrences(next);
