...
(function attachFuelGuardDomain(root, factory) {
  ...
  function interventionComparison({ intervention = {}, logs = [], targets = {}, weeks = 4, timeZone } = {}) {
    ...
    if (!interventionKey) return { direction: "insufficient", label: "Not enough dated information to compare this intervention." };
    const days = Math.max(7, Number(weeks || 4) * 7);
    ...
    if (!Number.isFinite(before.fuelling.averageGapMinutes) || !Number.isFinite(after.fuelling.averageGapMinutes)) {
      return { direction: "insufficient", before, after, label: "Not enough comparable fuel-gap data before and after this intervention." };
    }
