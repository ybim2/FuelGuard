...
Run `scripts/build-garmin-beta-release.sh` to build both beta IQ packages and export them to persistent local storage under `build/garmin-beta/0.4.3/`. Set `FUELGUARD_RELEASE_ROOT` only when you need a different persistent destination; the release script refuses `/tmp` and `/private/tmp` outputs.
...
4. Run `scripts/build-garmin-beta-release.sh` and upload the exported 0.4.3 Quick Log IQ package and the existing Activity Logger IQ package as new versions of their existing beta listings.
