...
test("Garmin API sends serially and removes only the acknowledged event", () => {
  ...
  assert.match(apiSource, /catch \(e\) \{\s*_inFlight = false;[\s\S]*FuelGuardTraining\.handleCommandResponse\(0, null\);[\s\S]*finishBatch\(\);/);
...
test("Quick Log registers and renders its pre-open glance without lifecycle network sync", () => {
  ...
  assert.doesNotMatch(appSource, /\(:glance\)\s*class FuelGuardQuickLogApp/);
  assert.match(appSource, /\(:glance\)\s*public function getGlanceView/);
  ...
  assert.match(glanceSource, /FuelGuardGlanceState\.summary\(\)/);
  assert.match(glanceSource, /FuelGuardGlanceState\.connected\(\)/);
  assert.match(glanceSource, /try \{[\s\S]*FuelGuardGlanceState\.summary\(\)[\s\S]*catch \(e\)/);
  assert.match(glanceSource, /getTextWidthInPixels/);
  assert.match(glanceSource, /Connect Fuel Guard/);
  assert.match(glanceStateSource, /LAST_SYNC_KEY/);
  assert.match(glanceStateSource, /TOKEN_KEY = "fg_device_token"/);
  assert.match(glanceStateSource, /LOCAL_FUEL_KEY/);
  ...
  assert.match(glanceStateSource, /Open FG to sync/);
  assert.match(glanceStateSource, /No fuel logged/);
  assert.match(glanceStateSource, /Fuelled just now/);
  assert.match(glanceStateSource, /Last fuel: /);
  assert.doesNotMatch(glanceStateSource, /MAX_STALE_SECONDS|5h\+ ago/);
  assert.doesNotMatch(glanceSource, /Open to log/);
});

test("Quick Log persists local Fuel for the glance and reconciles it with authenticated status", () => {
  ...
  const viewSource = readRepoFile("garmin/FuelGuard/quick-log/source/FuelGuardQuickLogView.mc");
  ...
  assertSourceOrder(viewSource, "FuelGuardQueue.enqueue(event)", "FuelGuardGlanceState.recordLocalFuel");
  ...
  assert.match(quickLogTests, /testFuelGuardQuickLogDurableOfflineFuelUpdatesGlanceImmediately/);
  assert.match(quickLogTests, /testFuelGuardQuickLogAcknowledgedFuelUpdatesGlance/);
  assert.match(quickLogTests, /testFuelGuardQuickLogOlderServerStatusCannotReplaceNewerLocalFuel/);
