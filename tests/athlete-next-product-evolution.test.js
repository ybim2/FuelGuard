...
test("new Athlete assets are included in the versioned offline app shell", () => {
  ...
  for (const asset of ["athlete-everyday-reflection", "athlete-analytics", "athlete-tools"]) {
    assert.match(html, new RegExp(`${asset}\\.css\\?v=mobile-pwa-v152-supplement-setup-save`));
    assert.match(html, new RegExp(`${asset}\\.js\\?v=mobile-pwa-v152-supplement-setup-save`));
