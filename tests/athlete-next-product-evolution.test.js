...
test("new Athlete assets are included in the versioned offline app shell", () => {
  ...
  for (const asset of ["athlete-everyday-reflection", "athlete-analytics", "athlete-tools"]) {
    assert.match(html, new RegExp(`${asset}\\.css\\?v=mobile-pwa-v146-social-share-preview`));
    assert.match(html, new RegExp(`${asset}\\.js\\?v=mobile-pwa-v146-social-share-preview`));
